...
public class Actions {
    ...
    /**
     * Reads an action specification and instantiates the required Action object and puts it on the actions list for execution later.
     *
     * @param str
    ...
    public void read(String str) throws IllegalArgumentException {
        if (!str.isEmpty()) {
            ...
            if (action == null) {
                throw new IllegalArgumentException(StartTrainRouteSearch.getProperty("unknown_action") + ": " + name);
            }
    ...
    /**
     * Constructs an Actions object reading the actions to be executed from an input stream
     *
     * @param stream
     * @throws IOException
     */
    ...
    public String execute(TrainNetwork trainNetwork) {
        ...
        for (Action action : actionsList) {
            s.append(action + " => " + action.execute(trainNetwork) + "\n");
        }
